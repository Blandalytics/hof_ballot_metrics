...
with st.sidebar:
    ...
    years_list = tracker_years['year'].sort_values(ascending=False).unique()
...
def ballot_chart(voter, year,pl_staff=pl_staff):
    ...
    big_misses = [x for x in player_options if voter_df[x].item() <= -0.15]
    ...
    ax1.text(1,-1,f'Voted For ({player_votes:.0f})',ha='right',color=pl_line_color)
    ax1.text(-1,-1,'Did Not Vote For',ha='left',color=pl_line_color)
...
pad1, col1, pad2 = st.columns([.125,.75,.125])
with col1:
